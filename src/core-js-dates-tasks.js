...
function formatDate(date) {
  ...
  let end = 'AM';
  const day = receiveDate.getUTCDate();
  const month = receiveDate.getUTCMonth() + 1;
  const year = receiveDate.getFullYear();
  const sec = receiveDate.getSeconds().toString().padStart(2, '0');
  const min = receiveDate.getMinutes().toString().padStart(2, '0');
  let hours = receiveDate.getUTCHours();
  if (hours > 12) {
    end = 'PM';
    hours %= 12;
  }
  ...
  return `${month}/${day}/${year}, ${hours}:${min}:${sec} ${end}`;
}
...
function getNextFridayThe13th(date) {
  if (date.getDay() === 5) {
    if (date.getDate() === 13) return date;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const dateStr = period.start.split('-').reverse().join('-');
  const end = new Date(endStr);
  const date = new Date(dateStr);
  const result = [];
  while (date < end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      const str = `${date.getUTCDate().toString().padStart(2, '0')}-${(date.getUTCMonth() + 1).toString().padStart(2, '0')}-${date.getFullYear()}`;
      result.push(str);
      date.setDate(date.getDate() + 1);
    }
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const check = new Date(year, 1, 29);
  return check.getMonth() === 1;
}
